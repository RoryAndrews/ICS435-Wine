...
def parameter_test(data):
    ...
    for key in data:
        ...
        target = data[key][1]
        print(key)
        result = best_SVR_parameters((features, target))
        print(result)
...
def best_SVR_parameters(data):
    ...
    dynamic_C_range = np.logspace(-2, 3, 8)
    dynamic_gamma_range = np.logspace(-2, 3, 8)
    dynamic_epsilon_range = np.linspace(0, 2, 8)
